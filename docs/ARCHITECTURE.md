...
| `prompt_sections.py` | Shared worker-prompt partials: `section_variables(search_dirs)` discovers every `*.md` file under a `worker_sections/` directory (package default `prompts/worker_sections/`, e.g. `scope_contract.md`, `issue_metadata.md`) and exposes each as a `section_<stem>` template value — repo-local `<search_dir>/worker_sections/` wins over the package default per filename, mirroring `prompts.resolve_template`. No section names are hardcoded; the available set is whatever `*.md` files exist on disk. `render_prompt()` folds these in and runs a two-pass substitution so section text carrying its own `$placeholders` resolves. **Section variants:** `section_variables(search_dirs, variants=...)` treats every subdirectory of a `worker_sections/` dir as an overlay — `worker_sections/<variant>/<stem>.md` replaces `worker_sections/<stem>.md` while that variant is active (searched before the base dir within each root, so a repo-local base partial still outranks the package's variant), and a stem that exists only inside a variant dir renders empty while it is inactive. |
| `prompt_skills.py` | Picks which section variant a worker/rework prompt renders under. `active_prompt_variants(worker_harness, repo_root, search_dirs)` returns `("skills",)` only when `HarnessCapabilities.skill_dirs` (`harnesses.py`) says the harness loads project skills **and** `repo_root` ships `<dir>/<name>/SKILL.md` for every skill named in the `worker_sections/skills/available_skills.md` bullet list (`declared_skills()` derives that set from the file, so the declaration and the loop text cannot drift); otherwise `()`, the plain git/gh loop. Slash-command skills belong to the consumer repo, not the harness. |
| `prompt_test_command.py` | Resolves the test command the worker/rework prompts splice in as `$targeted_test_command` / `$full_suite_command`: `dispatch.test_command` (runner prefix) first, else `derive_pytest_runner()` reads the repo-root `pyproject.toml` (core dependencies, then an extra, then a PEP 735 dependency group that declares `pytest`), else no command is invented and the prompt points at the repo's own `CLAUDE.md` / `CONTRIBUTING.md`. `prompt_test_command_values()` is the single supplier for both the worker (`orchestration/prompt_ops.py`) and rework (`rework_prompts.py`) writers. |
