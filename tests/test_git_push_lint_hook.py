"""Tests for ``scripts/git_push_lint_hook.py``'s ``main()`` (#1309).
...
machinery -- the tests here verify ``main()``'s deny/allow decisions and the
fail-open/fail-closed contract (plus the ``.claude/settings.json`` wiring),
not the reused machinery (which has its own coverage in
``test_worker_stop_gate.py``). The ``_is_git_push`` detector and the
``_resolve_cwd`` / ``_leading_cd_target`` cwd-resolution seam live in the
sibling modules ``test_git_push_lint_hook_is_git_push.py`` and
``test_git_push_lint_hook_resolve_cwd.py`` (Track-1 split, issue #1577).

``_load_stop_gate`` is monkeypatched to return a lightweight stand-in module
so no real ``ruff`` subprocesses run; the detached-HEAD fail-open test drives
a real ``git init``-ed ``tmp_path`` repo against the real stop gate.
"""
