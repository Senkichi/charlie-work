...
def test_worker_claude_code_md_renders_via_real_writer(tmp_path: Path) -> None:
    ...
    exactly what the api-worker dispatch path at workflow.py:8904-8908
    (and the matching path in the dispatch loop at workflow.py:9425-9427)
...
def test_review_md_renders_with_production_paths_and_no_state_dir_literal(
    ...
    mirroring workflow.py:10976's ``pr_dir = self.paths.prs / f"pr-{pr_number}"``
...
def test_review_md_repo_local_override_render_with_no_state_dir_literal(
    ...
    ``render_prompt`` (workflow.py:7548). That search-dir list is empty by
...
_CITATION_EXPECTATIONS: dict[tuple[int, int], str] = {
    ...
    (8904, 8908): "api_worker.worker_template",
    ...
    (7548, 7548): "render_prompt(template_name, values, search_dirs=self.prompt_dirs)",
}
