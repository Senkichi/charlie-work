...
The baseline is a directory of per-entry ``.count`` files (issue #1802), read
and written via ``charlie_work.ratchet_baseline``; these tests pin the mapping
values, not the on-disk encoding (that is ratchet_baseline's own test's job).
...
from charlie_work.ratchet_baseline import load_count_baseline
...
def _write_baseline_dir(path: Path, data: dict[str, int]) -> None:
    """Write a per-entry baseline dir under *path*."""
    for key, value in data.items():
        entry = path / (key + ".count")
        entry.parent.mkdir(parents=True, exist_ok=True)
        entry.write_text(f"{value}\n", encoding="utf-8")


def _read_baseline_dir(path: Path) -> dict[str, int]:
    return load_count_baseline(path)


def _entry_payloads(path: Path) -> list[str]:
    """Snapshot every entry file's content (for byte-identical assertions)."""
    return sorted(p.read_text(encoding="utf-8") for p in path.rglob("*") if p.is_file())
...
def test_lower_quantizes_and_only_lowers(refresh_mod, monkeypatch, tmp_path) -> None:
    ...
    # The dropped entry's file is removed, not zeroed -- a zeroed entry would
    # keep the dead path on the conflict surface.
    assert not (baseline_path / "src" / "under_cap.py.count").exists()
...
def test_lower_reports_no_changes_when_current(refresh_mod, monkeypatch, tmp_path, capsys) -> None:
    ...
    baseline_path = tmp_path / "file_size_ratchet_baseline"
    _write_baseline_dir(baseline_path, {"src/a.py": 1600, "src/b.py": 1000})
    original = _entry_payloads(baseline_path)
    monkeypatch.setattr(
        refresh_mod,
        "_scan_over_cap",
...
def test_lower_dry_run_does_not_write(refresh_mod, monkeypatch, tmp_path, capsys) -> None:
    baseline_path = tmp_path / "file_size_ratchet_baseline"
    _write_baseline_dir(baseline_path, {"src/a.py": 2000})
    original = _entry_payloads(baseline_path)
...
def test_check_fails_on_pending_lowerings_and_drops(
    ...
    apply. The baseline is left byte-identical -- --check never writes."""
    baseline_path = tmp_path / "file_size_ratchet_baseline"
    _write_baseline_dir(
        baseline_path,
...
def test_check_growth_only_is_not_a_fixed_point_violation(
    ...
    baseline_path = tmp_path / "file_size_ratchet_baseline"
    _write_baseline_dir(baseline_path, {"src/grown.py": 1000})
    original = _entry_payloads(baseline_path)
    monkeypatch.setattr(
        refresh_mod,
        "_scan_over_cap",
...
def test_check_fails_when_baseline_missing(refresh_mod, tmp_path, capsys) -> None:
    rc = refresh_mod._check(tmp_path, tmp_path / "file_size_ratchet_baseline")

    assert rc == 1
    assert "--init" in capsys.readouterr().err


def test_check_fails_closed_on_malformed_entry(refresh_mod, tmp_path, capsys) -> None:
    """Issue #1802 fail-closed: a malformed .count entry must exit non-zero,
    never silently load as zero."""
    baseline_path = tmp_path / "file_size_ratchet_baseline"
    _write_baseline_dir(baseline_path, {"src/a.py": 1000})
    (baseline_path / "src" / "a.py.count").write_text("oops\n", encoding="utf-8")
...
def test_main_routes_check_flag_to_check_not_lower(refresh_mod, monkeypatch, tmp_path) -> None:
    ...
    assert called["check"] == tmp_path / "file_size_ratchet_baseline"
