...
_PRIVATE_SLUG_BASELINE_FILENAME = ".private-slug-baseline.json"
...
def _regenerate_private_slug_baseline(
    ...
    tracked_files = [
        f for f in ls_result.stdout.splitlines() if f and f != _PRIVATE_SLUG_BASELINE_FILENAME
    ]
...
def run_private_slug_check_command(args: argparse.Namespace) -> CommandResult:
    ...
    """
    ctx = bootstrap_command(args)
    baseline_path = ctx.repo_root / _PRIVATE_SLUG_BASELINE_FILENAME
    ...
    base_show = run_captured(
        ["git", "show", f"{base}:{_PRIVATE_SLUG_BASELINE_FILENAME}"],
        cwd=ctx.repo_root,
        timeout_seconds=30,
    )
    if base_show.ok:
    ...
    delta = find_slug_mentions_in_diff(
        diff_result.stdout,
        slugs,
        exclude_paths=frozenset({_PRIVATE_SLUG_BASELINE_FILENAME}),
    )
    ...
    if delta.net_new > 0 and baseline_increase < delta.net_new:
        ...
        message = (
            ...
            f"in {_PRIVATE_SLUG_BASELINE_FILENAME} to acknowledge the new mentions, "
