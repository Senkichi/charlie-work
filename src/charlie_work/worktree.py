    ...
    *,
    ...
    if _has_origin_remote(repo_root) and not _commit_exists_locally(repo_root, head_sha):
        ...
        if not fetch_result.ok:
            ...
            # branch). The PR head ref is always advertised — fetching it can
            # pull head_sha in without changing WHICH sha gets reviewed; the
            # checkout below still pins the exact caller-supplied head_sha.
            run_captured(
